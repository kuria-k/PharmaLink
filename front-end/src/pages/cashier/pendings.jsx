...
//                           KES{" "}
//                           {parseFloat(p.sale?.total_amount).toLocaleString()}
//                         </span>
//                       </td>
...
//     <span className="font-semibold text-gray-800">
//       {selectedPayment.sale?.customer_name}
//     </span>
//   </div>
...
//     <span className="font-semibold text-gray-800">
//       {selectedPayment.sale?.posted_by}
//     </span>
//   </div>
...
//       KES{" "}
//       {parseFloat(selectedPayment.sale?.total_amount).toLocaleString()}
//     </span>
//   </div>
// </div>
...
//                     Leave empty for auto-generated receipt
//                   </p>
//                 </div>
//               )}
...
//                     <>
//                       <svg
//                         className="animate-spin inline h-5 w-5 mr-2"
...
//                   KES{" "}
//                   {parseFloat(
//                     formData.amount_paid || selectedPayment?.sale?.total_amount
//                   ).toLocaleString()}
//                 </p>
//               </div>
...
const PendingPayments = ({ currentUser }) => {
  const [payments, setPayments] = useState([]);
  const [selectedPayment, setSelectedPayment] = useState(null);
  const [postedByUser, setPostedByUser] = useState(null); // ✅ Added
  const [formData, setFormData] = useState({
    amount_paid: "",
    payment_mode: "cash",
    mpesa_receipt: "",
                      ...
                      <td className="px-4 py-4 text-center">
                        <button
                          onClick={async () => {
                            ...
                            if (p.sale?.posted_by) {
                              await fetchPostedByUser(p.sale.posted_by);
                            }
                          }}
                          className="bg-gradient-to-r from-[#B57C36] to-[#a56b2f] text-white px-6 py-2 rounded-xl hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200 font-medium"
                        >
              ...
              <div className="bg-[#B57C36]/5 rounded-xl p-4 space-y-3 border border-[#B57C36]/20">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-[#B57C36]/20">
                  ...
                  <span className="font-bold text-[#B57C36] text-xl">
                    KES{" "}
                    {parseFloat(
                      selectedPayment.sale?.total_amount
                    ).toLocaleString()}
                  </span>
                </div>
              </div>
