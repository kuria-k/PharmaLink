...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (editingId) {
      ...
      await axios.patch(
        `http://localhost:8000/api/inventory/products/${editingId}/`,
        payload
      );
      setMessage("Product updated successfully!");
    } else {
      // ✅ Use upsert endpoint so existing SKU adds quantity instead of erroring
      await axios.post(
        "http://localhost:8000/api/inventory/products/upsert/",
        payload
      );
    ...
    if (err.response?.data?.sku) {
      setMessage("This SKU already exists. Stock has been added to the existing product.");
    } else {
      setMessage(
    ...
    <div className="p-6">
      <div className="glass p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Basic Info */}
          <div>
            <h2 className="text-lg font-semibold text-[#B57C36] mb-2">
              Basic Info
            </h2>
            <div className="grid grid-cols-2 gap-4">
              <input
                ...
                className="p-3 rounded-lg bg-white/50 border border-[#B57C36]/40"
              />
              <input
                type="text"
                name="sku"
                ...
                value={formData.sku || ""}
                readOnly
                className="p-3 rounded-lg bg-gray-100 border border-[#B57C36]/40"
              />
              <input
                type="text"
                name="category"
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <input
                ...
                placeholder="Buying Price per Piece (optional)"
              ...
              <input
                ...
                value={formData.selling_price_per_pack}
                onChange={handleChange}
                className="p-3 rounded-lg bg-white/50 border border-[#B57C36]/40"
              />
              <input
                ...
                placeholder="Selling Price per Piece (optional)"
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <input
                ...
                onChange={handleChange}
                min="0"
                className="p-3 rounded-lg bg-white/50 border border-[#B57C36]/40 focus:ring-2 focus:ring-[#B57C36]"
              />
              <input
                type="number"
                name="lead_time"
